/*
 * drivers/cpufreq/cpufreq_rebirthx.c
 *
 ...
 * Modified by: Yousef Algadri (yusufgadrie@gmail.com)
 *
 */
...
#include <trace/events/cpufreq_rebirthx.h>

struct cpufreq_rebirthx_policyinfo {
	...
	struct cpufreq_rebirthx_tunables *cached_tunables;
	struct sched_load *sl;
};

/* Protected by per-policy load_lock */
struct cpufreq_rebirthx_cpuinfo {
	u64 time_in_idle;
...
static DEFINE_PER_CPU(struct cpufreq_rebirthx_policyinfo *, polinfo);
static DEFINE_PER_CPU(struct cpufreq_rebirthx_cpuinfo, cpuinfo);
...
struct cpufreq_rebirthx_tunables {
	int usage_count;
...
#define DEFAULT_SCREEN_OFF_MAX 300000
	unsigned long screen_off_max;
};

/* For cases where we have single governor instance for system */
static struct cpufreq_rebirthx_tunables *common_tunables;
static struct cpufreq_rebirthx_tunables *cached_common_tunables;

static struct attribute_group *get_sysfs_attr(void);

/* Round to starting jiffy of next evaluation window */
static u64 round_to_nw_start(u64 jif,
			     struct cpufreq_rebirthx_tunables *tunables)
{
...
static void cpufreq_rebirthx_timer_resched(unsigned long cpu,
					      bool slack_only)
{
	struct cpufreq_rebirthx_policyinfo *ppol = per_cpu(polinfo, cpu);
	struct cpufreq_rebirthx_cpuinfo *pcpu;
	struct cpufreq_rebirthx_tunables *tunables =
 ...
 * this function.
 */
static void cpufreq_rebirthx_timer_start(
	struct cpufreq_rebirthx_tunables *tunables, int cpu)
{
	struct cpufreq_rebirthx_policyinfo *ppol = per_cpu(polinfo, cpu);
	struct cpufreq_rebirthx_cpuinfo *pcpu;
...
static unsigned int freq_to_targetload(
	struct cpufreq_rebirthx_tunables *tunables, unsigned int freq)
{
	int i;
	unsigned int ret;
	unsigned long flags;
 ...
 */
static unsigned int choose_freq(struct cpufreq_rebirthx_policyinfo *pcpu,
		unsigned int loadadjfreq)
{
...
{
	struct cpufreq_rebirthx_policyinfo *ppol = per_cpu(polinfo, cpu);
	struct cpufreq_rebirthx_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
	struct cpufreq_rebirthx_tunables *tunables =
		ppol->policy->governor_data;
	u64 now;
	u64 now_idle;
...
static unsigned int sl_busy_to_laf(struct cpufreq_rebirthx_policyinfo *ppol,
				   unsigned long busy)
{
	int prev_load;
	struct cpufreq_rebirthx_tunables *tunables =
...
static void cpufreq_rebirthx_timer(unsigned long data)
{
	s64 now;
	...
	int cpu_load;
	int pol_load = 0;
	struct cpufreq_rebirthx_policyinfo *ppol = per_cpu(polinfo, data);
	struct cpufreq_rebirthx_tunables *tunables =
		ppol->policy->governor_data;
	...
	for_each_cpu(cpu, ppol->policy->cpus) {
		...
		trace_cpufreq_rebirthx_cpuload(cpu, cpu_load, new_load_pct,
						  prev_l, pred_l);
	...
	if (ppol->target_freq == new_freq &&
		...
		trace_cpufreq_rebirthx_already(
			max_cpu, pol_load, ppol->target_freq,
			ppol->policy->cur, new_freq);
		spin_unlock_irqrestore(&ppol->target_freq_lock, flags);
		goto rearm;
	...
	trace_cpufreq_rebirthx_target(max_cpu, pol_load, ppol->target_freq,
					 ppol->policy->cur, new_freq);
...
rearm:
	if (!timer_pending(&ppol->policy_timer))
		cpufreq_rebirthx_timer_resched(data, false);
...
static int cpufreq_rebirthx_speedchange_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
	struct cpufreq_rebirthx_policyinfo *ppol;
	struct cpufreq_rebirthx_tunables *tunables;
	...
	while (1) {
		...
		for_each_cpu(cpu, &tmp_mask) {
			...
			trace_cpufreq_rebirthx_setspeed(cpu,
						     ppol->target_freq,
						     ppol->policy->cur);
			up_read(&ppol->enable_sem);
		}
...
static void cpufreq_rebirthx_boost(struct cpufreq_rebirthx_tunables *tunables)
{
	int i;
	int anyboost = 0;
...
static enum hrtimer_restart cpufreq_rebirthx_hrtimer(struct hrtimer *timer)
{
	struct cpufreq_rebirthx_policyinfo *ppol = container_of(timer,
			struct cpufreq_rebirthx_policyinfo, notif_timer);
	int cpu;
	...
	cpu = ppol->notif_cpu;
	trace_cpufreq_rebirthx_load_change(cpu);
	del_timer(&ppol->policy_timer);
	del_timer(&ppol->policy_slack_timer);
	cpufreq_rebirthx_timer(cpu);
...
static int cpufreq_rebirthx_notifier(
...
static struct notifier_block cpufreq_notifier_block = {
	.notifier_call = cpufreq_rebirthx_notifier,
};
...
static ssize_t show_above_hispeed_delay(
	struct cpufreq_rebirthx_tunables *tunables, char *buf)
{
	int i;
	ssize_t ret = 0;
	unsigned long flags;
...
static ssize_t show_hispeed_freq(struct cpufreq_rebirthx_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->hispeed_freq);
}

static ssize_t store_hispeed_freq(struct cpufreq_rebirthx_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
...
static ssize_t show_go_hispeed_load(struct cpufreq_rebirthx_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%lu\n", tunables->go_hispeed_load);
}

static ssize_t store_go_hispeed_load(struct cpufreq_rebirthx_tunables
...
static ssize_t show_min_sample_time(struct cpufreq_rebirthx_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%lu\n", tunables->min_sample_time);
}

static ssize_t store_min_sample_time(struct cpufreq_rebirthx_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_timer_rate(struct cpufreq_rebirthx_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%lu\n", tunables->timer_rate);
}

static ssize_t store_timer_rate(struct cpufreq_rebirthx_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val, val_round;
	struct cpufreq_rebirthx_tunables *t;
	int cpu;
...
static ssize_t show_timer_slack(struct cpufreq_rebirthx_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%d\n", tunables->timer_slack_val);
}

static ssize_t store_timer_slack(struct cpufreq_rebirthx_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_boost(struct cpufreq_rebirthx_tunables *tunables,
			  char *buf)
{
	return sprintf(buf, "%d\n", tunables->boost_val);
}

static ssize_t store_boost(struct cpufreq_rebirthx_tunables *tunables,
			   const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	if (tunables->boost_val) {
		trace_cpufreq_rebirthx_boost("on");
		if (!tunables->boosted)
			cpufreq_rebirthx_boost(tunables);
	} else {
		tunables->boostpulse_endtime = ktime_to_us(ktime_get());
		trace_cpufreq_rebirthx_unboost("off");
	}

	return count;
}

static ssize_t store_boostpulse(struct cpufreq_rebirthx_tunables *tunables,
				const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	trace_cpufreq_rebirthx_boost("pulse");
	if (!tunables->boosted)
		cpufreq_rebirthx_boost(tunables);
	return count;
}

static ssize_t show_boostpulse_duration(struct cpufreq_rebirthx_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%d\n", tunables->boostpulse_duration_val);
}

static ssize_t store_boostpulse_duration(struct cpufreq_rebirthx_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_io_is_busy(struct cpufreq_rebirthx_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->io_is_busy);
}

static ssize_t store_io_is_busy(struct cpufreq_rebirthx_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static int cpufreq_rebirthx_enable_sched_input(
			struct cpufreq_rebirthx_tunables *tunables)
{
	int rc = 0, j;
	struct cpufreq_rebirthx_tunables *t;
...
static int cpufreq_rebirthx_disable_sched_input(
			struct cpufreq_rebirthx_tunables *tunables)
{
	mutex_lock(&sched_lock);
			...
			const char *buf, size_t count)
{
	...
	if (val)
		ret = cpufreq_rebirthx_enable_sched_input(tunables);
	else
		ret = cpufreq_rebirthx_disable_sched_input(tunables);
...
static ssize_t show_powersave_bias(struct cpufreq_rebirthx_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->powersave_bias);
}
 static ssize_t store_powersave_bias(struct cpufreq_rebirthx_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static struct attribute *rebirthx_attributes_gov_sys[] = {
...
static struct attribute_group rebirthx_attr_group_gov_sys = {
	.attrs = rebirthx_attributes_gov_sys,
	.name = "rebirthx",
};

/* Per policy governor instance */
static struct attribute *rebirthx_attributes_gov_pol[] = {
...
static struct attribute_group rebirthx_attr_group_gov_pol = {
	.attrs = rebirthx_attributes_gov_pol,
	.name = "rebirthx",
};
...
static struct attribute_group *get_sysfs_attr(void)
{
	if (have_governor_per_policy())
		return &rebirthx_attr_group_gov_pol;
	else
		return &rebirthx_attr_group_gov_sys;
}

static void cpufreq_rebirthx_nop_timer(unsigned long data)
{
}

static struct cpufreq_rebirthx_tunables *alloc_tunable(
					struct cpufreq_policy *policy)
{
	struct cpufreq_rebirthx_tunables *tunables;
...
static struct cpufreq_rebirthx_policyinfo *get_policyinfo(
					struct cpufreq_policy *policy)
{
	struct cpufreq_rebirthx_policyinfo *ppol =
	...
	ppol->policy_timer.function = cpufreq_rebirthx_timer;
	init_timer(&ppol->policy_slack_timer);
	ppol->policy_slack_timer.function = cpufreq_rebirthx_nop_timer;
	hrtimer_init(&ppol->notif_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	ppol->notif_timer.function = cpufreq_rebirthx_hrtimer;
...
static struct cpufreq_rebirthx_tunables *get_tunables(
				struct cpufreq_rebirthx_policyinfo *ppol)
{
	if (have_governor_per_policy())
...
static int cpufreq_governor_rebirthx(struct cpufreq_policy *policy,
		unsigned int event)
{
	int rc;
	struct cpufreq_rebirthx_policyinfo *ppol;
	...
	case CPUFREQ_GOV_POLICY_INIT:
		...
		if (tunables->use_sched_load)
			cpufreq_rebirthx_enable_sched_input(tunables);
	...
	case CPUFREQ_GOV_POLICY_EXIT:
		...
		if (tunables->use_sched_load)
			cpufreq_rebirthx_disable_sched_input(tunables);
	...
	case CPUFREQ_GOV_START:
		...
		ppol->last_evaluated_jiffy = get_jiffies_64();
		cpufreq_rebirthx_timer_start(tunables, policy->cpu);
	...
	case CPUFREQ_GOV_LIMITS:
		...
		if (ppol->governor_enabled) {
			if (policy->min < ppol->min_freq)
				cpufreq_rebirthx_timer_resched(policy->cpu,
								  true);
			ppol->min_freq = policy->min;
		}
		up_read(&ppol->enable_sem);
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_rebirthx
static
#endif
struct cpufreq_governor cpufreq_gov_rebirthx = {
	.name = "rebirthx",
	.governor = cpufreq_governor_rebirthx,
	.max_transition_latency = 10000000,
	.owner = THIS_MODULE,
};

static int __init cpufreq_rebirthx_init(void)
{
	...
	speedchange_task =
		kthread_create(cpufreq_rebirthx_speedchange_task, NULL,
			       "cfrebirthx");
	...
	ret = cpufreq_register_governor(&cpufreq_gov_rebirthx);
	if (ret) {
...
#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_rebirthx
fs_initcall(cpufreq_rebirthx_init);
#else
module_init(cpufreq_rebirthx_init);
#endif

static void __exit cpufreq_rebirthx_exit(void)
{
	int cpu;

	cpufreq_unregister_governor(&cpufreq_gov_rebirthx);
	kthread_stop(speedchange_task);
	put_task_struct(speedchange_task);
...
module_exit(cpufreq_rebirthx_exit);

MODULE_AUTHOR("Mike Chan <mike@android.com>");
MODULE_DESCRIPTION("'cpufreq_rebirthx' - A cpufreq governor for "
